...
int proto_init_cmd_rcsocket(char *URL,int port){
    ...
    if(cmd_sock == SOCKET_CONNECT_ERROR){
        printf("Error on connect %s:%d",URL,port);
    ...
    if(sendsize != MAX_CMD_PRO_LEN ){
        printf("error on send init cmd!!!\n");
        return PROTO_INIT_CMD_RCSOCKET_ERROR;
    }
    // check server create OK ?
    readsize = API_socket_recv(cmd_sock,cmd,MAX_CMD_PRO_LEN);
    if(readsize != MAX_CMD_PRO_LEN){
        printf("I donot know what server said\n");
        return PROTO_INIT_CMD_RCSOCKET_ERROR;
    }
    if(cmd_buff[0]!= PRO_MAKE_NET &&
         cmd_buff[1]!=CONFIRM_YOU_ARE_SOCK_CLIENT ){
        printf("take a look if server said OK ?\n");
        return PROTO_INIT_CMD_RCSOCKET_ERROR;
    }
    return cmd_sock;
}
...
int proto_get_rcsocket(char *URL,int port,int poolnum){
    ...
    itochar(poolnum,buf,4);
    cmd[2] = buf[0];
    cmd[3] = buf[1];
    cmd[4] = buf[2];
    cmd[5] = buf[3];
    sendsize = API_socket_send(rcsock,cmd_buff,MAX_CMD_PRO_LEN);
    if(sendsize != MAX_CMD_PRO_LEN){
